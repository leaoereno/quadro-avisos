        ...
        <div class="qa-dash-grid" id="qa-grid">
            <?php foreach ($data['avisos'] as $aviso): ?>
                <div class="qa-card qa-card--<?= htmlspecialchars($aviso['tipo_borda']) ?> qa-card-clickable"
                     ...
                     data-fim="<?= _('Until') ?> <?= (new DateTime($aviso['fim']))->format('d/m/Y H:i') ?>"
                    ...
                    <div class="qa-card-header">
                        ...
                        <span class="qa-validity-chip">
                            <?= _('Until') ?> <?= (new DateTime($aviso['fim']))->format('d/m/Y H:i') ?>
                        </span>
                    </div>
                    ...
                    <div class="qa-card-expand-hint"><?= _('Click to expand') ?> ↗</div>
                </div>
            <?php endforeach ?>
        </div>
    <?php endif ?>
...
<div id="qa-modal-overlay" class="qa-modal-overlay" onclick="qaCloseModal(event)">
    <div class="qa-modal" id="qa-modal">
        <div class="qa-modal-header" id="qa-modal-header">
            ...
            <button class="qa-modal-close" onclick="qaCloseModal(null)" title="<?= _('Close') ?>">✕</button>
        </div>
...
<style>
.qa-card-clickable { cursor:pointer; transition:box-shadow .2s,transform .15s; }
.qa-card-clickable:hover { box-shadow:0 6px 20px rgba(0,0,0,.18); transform:translateY(-2px); }
.qa-card-expand-hint { font-size:11px; color:var(--color-text-secondary,#aaa); text-align:right; margin-top:4px; opacity:0; transition:opacity .2s; }
.qa-card-clickable:hover .qa-card-expand-hint { opacity:1; }
.qa-modal-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.55); z-index:9999; align-items:center; justify-content:center; padding:20px; backdrop-filter:blur(2px); }
.qa-modal-overlay.qa-open { display:flex; animation:qaFadeIn .15s ease; }
@keyframes qaFadeIn { from{opacity:0} to{opacity:1} }
.qa-modal { background:var(--color-bg-surface,#fff); border-radius:8px; box-shadow:0 8px 40px rgba(0,0,0,.3); max-width:780px; width:100%; max-height:85vh; display:flex; flex-direction:column; animation:qaSlideUp .18s ease; border-top:5px solid var(--qa-modal-color,#1a7abf); }
@keyframes qaSlideUp { from{transform:translateY(20px);opacity:0} to{transform:translateY(0);opacity:1} }
.qa-modal-header { display:flex; align-items:center; justify-content:space-between; padding:16px 20px 0; gap:10px; flex-wrap:wrap; }
.qa-modal-meta { display:flex; gap:8px; align-items:center; flex-wrap:wrap; }
.qa-modal-close { background:none; border:none; font-size:18px; cursor:pointer; color:var(--color-text-secondary,#888); padding:4px 8px; border-radius:4px; line-height:1; transition:background .15s,color .15s; margin-left:auto; }
.qa-modal-close:hover { background:rgba(0,0,0,.08); color:var(--color-text-main,#333); }
.qa-modal-title { font-size:20px; font-weight:600; margin:12px 20px 0; color:var(--color-text-main,#222); line-height:1.3; }
.qa-modal-body { flex:1; overflow-y:auto; padding:14px 20px; font-size:14px; line-height:1.6; color:var(--color-text-main,#333); }
.qa-modal-footer { display:flex; gap:16px; padding:12px 20px; border-top:1px solid var(--color-border,#e8e8e8); font-size:12px; color:var(--color-text-secondary,#888); flex-wrap:wrap; }
.qa-modal--info{--qa-modal-color:#1a7abf} .qa-modal--success{--qa-modal-color:#3a8a3a}
.qa-modal--warning{--qa-modal-color:#c89200} .qa-modal--danger{--qa-modal-color:#c0392b}
.qa-modal--mudanca{--qa-modal-color:#7b5ea7} .qa-modal--evento{--qa-modal-color:#0097a7}
</style>
...
(function() {
    ...
    window.qaOpenModal = function(card) {
        loadMarked(function() {
            ...
            document.getElementById('qa-modal-badge').textContent  = card.dataset.badge || '';
            document.getElementById('qa-modal-badge').className     = 'qa-badge qa-badge--' + tipo;
            document.getElementById('qa-modal-fim').textContent     = card.dataset.fim || '';
            document.getElementById('qa-modal-title').textContent   = card.dataset.titulo || '';
            ...
            document.getElementById('qa-modal-usuario').textContent = '👤 ' + (card.dataset.usuario || '');
            document.getElementById('qa-modal-criado').textContent  = '🕐 ' + (card.dataset.criado || '');
